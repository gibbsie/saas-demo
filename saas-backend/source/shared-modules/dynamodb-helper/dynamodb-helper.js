 ...
 * @param mediaUpdateParams The parameters for the update
 * @param credentials User credentials
 * @param callback Callback with results
 */
DynamoDBHelper.prototype.updateItem = function(mediaUpdateParams, credentials, callback) {
    this.getDynamoDBDocumentClient(credentials, function (error, docClient) {
        docClient.update(mediaUpdateParams, function(err, data) {
            if (err) {
                callback(err);
            } else {
