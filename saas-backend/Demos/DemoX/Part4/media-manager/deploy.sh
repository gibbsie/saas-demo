...
CODEBUILD_PROJECT="saas-demo-media-svc"
CODE_PIPELINE="saas-demo-media-svc"
SAAS_BACKEND_BUCKET=$(aws ssm get-parameter --name "saas-demo-cfnbucket-${AWS_REGION}" | jq -r '.Parameter.Value')
DEPLOY_MONITOR_QUEUE=$(aws ssm get-parameter --name "saas-demo-ci-cd-queue-media-${AWS_REGION}" | jq -r '.Parameter.Value')
...
if [ -z "$DEPLOY_MONITOR_QUEUE" ]; then
    echo "Error: Can't determine SaaS OctankHollywood SQS queue URL"
    exit 1
fi

echo "SaaS OctankHollywood S3 bucket: $SAAS_BACKEND_BUCKET"
echo "SaaS OctankHollywood SQS queue: $DEPLOY_MONITOR_QUEUE"
echo
...
aws s3 cp server.js s3://${SAAS_BACKEND_BUCKET}/source/media-manager/
